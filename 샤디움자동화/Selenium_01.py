...
M.TIME(3)
(
action.send_keys((Keys.TAB)*11).key_down(Keys.ENTER).key_up(Keys.ENTER).pause(4)
.perform()
)
...
for k in range(5):
    ...
    # 13 - 1~15 랜덤숫자 기입(소숫점 1자리까지)
    driver.find_element(By.CLASS_NAME,"sc-fTFMiz.kclJa.token-amount-input").send_keys("2.5")#난수 숫자 기입 #From 클릭
    M.TIME(4)
...
for j in range(5):
    # 18 - 샤디움 2.X 트젝하기 (5회 반복)
    driver.get("https://app.swapped.finance/swap") #스왑 사이트로 이동
    M.TIME(5)
    M.CLICK_MOSUE(1900, 700) #빈공백 아무곳이나 클릭
    action.send_keys(Keys.TAB).send_keys("5.5").pause(7).perform()#4~15난수입력해주세요
    action.reset_actions() #액션 종료
...
action.send_keys(Keys.TAB).send_keys(Keys.ENTER).pause(2).send_keys((Keys.TAB) * 2).send_keys(Keys.ENTER).pause(6).perform()
action.reset_actions() #액션 종료
